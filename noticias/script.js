...
function showNexts() {
    ...
    if (filtered && document.documentElement.clientWidth <= 500 ){
        nodes = document.querySelector('#filtered-grids').querySelectorAll('.news__grid-MB__new');
        deviceDisplay="flex";
    }
       

    else if (filtered && document.documentElement.clientWidth > 500 ){
...
function createRowNewsFilteredMB(id){
    ...
    <div class="news__grid-MB__info">
        <div class="news__grid-MB__date">
            <span>${getContent(id,ini1)}</span>
        </div>
        <div class="news__grid-MB__title">${getContent(id,ini1)}</div>
    </div>
</div>`
}
